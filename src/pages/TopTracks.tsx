...
const TopTracks = () => {
  ...
  const [_filteredArtist, setFilteredArtist] = useState<Artist[]>([]);
  ...
  const totalPlayCount = artists.reduce(
    (acc, artist) => acc + artist.playcount,
    0,
  );
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-4 items-stretch">
              <StatCard
                ...
                icon={
                  <Music
                    className="bg-pink/10 rounded-lg text-pink"
                    size={30}
                  />
                }
              />
              <StatCard
                ...
                icon={
                  <Play
                    className="bg-green-400/20  rounded-lg text-green-400"
                    size={30}
                  />
                }
              />
              <StatCard
                ...
                icon={
                  <TrendingUp
                    className="bg-purple/10  rounded-lg text-purple"
                    size={30}
                  />
                }
              />
          ...
          <div className="flex justify-around items-center relative mb-8 gap-x-4">
            ...
            <div className="flex ml-auto gap-2">
              <button
                ...
                className="rounded-lg border  border-gray-100/10 text-white border-1/2 px-2 py-1 w-40 items-center hover:bg-teal flex items-center justify-between bg-sidebar"
              >
                {isSelected}
                <ChevronDown size={20} />
              </button>
              {sortFilter && (
                <div className="absolute top-12 right-0 bg-card p-2 rounded-lg">
                  <ul>
                    <li
                      onClick={() => setisSelected("Most Streamed")}
                      className="hover:bg-teal flex items-center p-2 rounded-lg cursor-pointer gap-2"
                    >
                      Most Streamed
                      <Check
                        size={18}
                        className={`mr-2 text-gray-100/20 ${isSelected === "Most Streamed" ? "visible" : "invisible"} `}
                      />
                    </li>

                    <li
                      onClick={() => setisSelected("Track Name")}
                      className="hover:bg-teal flex items-center p-2 rounded-lg cursor-pointer gap-2"
                    >
                      {" "}
                      Track Name
                      <Check
                        size={18}
                        className={`mr-2 text-gray-100/20 ${isSelected === "Track Name" ? "visible" : "invisible"} `}
                      />{" "}
                    </li>
                  </ul>
                </div>
                ...
                >
                  ...
                  <td className=" px-4 py-4 text-left">
                    <div className="flex gap-2 items-center justify-left">
                      <span
                        className="block shrink-0  w-1 h-8 bg-teal rounded-lg "
                        style={{

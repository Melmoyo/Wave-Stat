    ...
    <div className=" min-h-screen p-8">
      <div>
        ...
        <p className="text-xl mb-8 text-gray-200">
          Side-by-side comparison of up to 4 artists
        </p>
      </div>
      {/*Buttons*/}
      <div className="flex gap-x-8 ml-auto ">
        {selectedArtist.map((sa: string, index) => (
          <button
            ...
            key={sa}
            className="flex items-center rounded-3xl border  w-40 p-2 justify-between"
          >
            <div
              style={{
                borderColor: colors[index % colors.length],
              ...
              className=" rounded-full w-2 h-2 flex p-4 justify-center items-center mr-2"
            >
              {sa.slice(0, 1)}
            </div>
            {sa}
            <X size={20} onClick={() => handleRemove(sa)} className="mx-auto" />
          </button>
        ))}
        ...
        <div className="relative">
          {selectedArtist.length < 4 && (
            <button
              ...
              className="flex items-center rounded-lg border border-white w-30 p-2 hover:bg-teal"
            >
              <Plus size={20} />
              Add Artist
              <ChevronDown size={20} />
        ...
        <div className="  mt-4 p-8 grid grid-cols-2 gap-20">
          <div className=" p-4 rounded-lg bg-card">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart
                responsive
                data={selectedArtist.map(
                  (sa: string) => artists.find((a) => a.name === sa) ?? {},
                )}
                layout="vertical"
              >
                <XAxis
                ...
                />
                <Bar dataKey="listeners" width="20%">
                  {selectedArtist.map((_, index) => (
            ...
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Tooltip
                  ...
                  itemStyle={{ color: "white" }}
                />
                <Bar dataKey="playcount" width="30%">
                  {selectedArtist.map((_, index) => (
                    <Cell key={index} fill={colors[index % colors.length]} />
                  ))}
                </Bar>
